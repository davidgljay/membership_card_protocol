...
**1.3 IPFS + MinIO services** — [Haiku]
- What: add `ipfs/kubo` (single node, per decision) and `minio/minio` services with healthchecks and named volumes; create the press's expected S3 bucket via a `mc` init container.
- Context: compose file from 1.2; press's S3 usage (`press/server`, grep for S3 client config to learn bucket/endpoint env vars).
- Done when: `ipfs id` and MinIO healthcheck pass; bucket exists on startup.
...
- Done when: Synapse healthcheck passes; a test user can register and create a room via the client-server API; policy module logs show it loaded.

**1.6 Press under workerd** — [Sonnet]
- What: author `press/wrangler.toml` (nodejs_compat, vars for chain RPC, IPFS API, MinIO endpoint/creds, `EXTERNAL_KV_URL`); build with `NITRO_PRESET=cloudflare-module`; run under `wrangler dev` in a container. Wire env to the stack's services.
- Context: `press/nitro.config.ts`, `press/package.json`, `wallet-service/wrangler.toml` (as the pattern), `specs/object_specs/press.md`, decisions: workerd fidelity + MinIO for S3 (strategic plan §Rationale Goal 1, §Open Questions 2).
- Done when: press health endpoint responds from inside the compose network, running under workerd, and can reach chain, IPFS, and MinIO.

**1.7 Wallet-service under workerd** — [Haiku]
- What: replicate 1.6's container pattern for wallet-service using its existing `wrangler.toml` (miniflare provides WALLET_KV locally); wire env vars for Synapse, chain, IPFS.
- Context: 1.6's Dockerfile/pattern, `wallet-service/wrangler.toml`, `wallet-service/nitro.config.ts`.
- Done when: wallet-service health endpoint responds under workerd with KV reads/writes working.
