...
- **Next action:** Phase 5 (Client-SDK Integration), starting with Step 17 (evaluate/select the Matrix crypto binding — a checkpoint requiring David's sign-off before any client-sdk crypto code is written) and Step 17a (join-attestation construction/signing — needed before Step 18's "joining without a valid attestation actually fails end-to-end" can be tested for real, which Phase 4's own milestone review flagged as not yet exercised).
