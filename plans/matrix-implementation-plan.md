...
## Current State (end of day 2026-07-11 — read this first if picking up fresh)
...
- **Phase 3 (Synapse policy module): not started.** But its two riskiest steps (9c, 10) were substantially de-risked by separate work on the verifier packages (below) — **read Steps 9 and 10's current text in full before starting anything in Phase 3**, they were rewritten today to reflect this and are not what they were when Phase 3 was first planned.
- **Separate but load-bearing work, done today: `membership-card-verifier`/`@membership-card-protocol/verifier` gained two new features** (`plans/verifier-criteria-and-chain-plan.md`, commits `2709e500`, `372d88fc`, `2258b908`): `return_chain` (exposes the full chain — addresses, pubkeys, decrypted card content — on `verify_envelope`/`verify_card` results) and `policy_match`/`conditions` (checks whether a chain includes a card issued under a given policy, with field-match conditions, exact-match or regex — exported as a standalone `evaluate_policy_match`/`evaluatePolicyMatch` function too, not just embedded in the result). Both packages: TS 101 tests, Python 130 tests (including 5 cross-language interop cases), `tsc`/`mypy` clean. **This is why Phase 3's Step 9c is now much smaller than originally planned** — the room predicate document's `any_of`-over-policies shape turned out to be exactly what `conditions` checks one entry at a time, so `predicates.py` becomes a thin loop over the exported function rather than a general predicate-tree evaluator.
- **One remaining loose end, not yet resolved as of this note:** `membership_card_verifier/package.json` (the root workspace file) had its `name`/`private`/`version` fields changed for npm publish prep, by a process outside this conversation (commit `11e76bb6`, already landed). The Python package (`membership-card-verifier` on PyPI) was, as of this note, still awaiting publish approval — confirm current status before assuming either package is live on a public registry; Step 9's text has a reminder to check this before depending on a versioned package reference instead of a path/git one.
- **Next action:** start Phase 3, Step 8 (scaffold the module package), then Step 9a/9b/9c and Step 10 in the order and with the "Who" assignments their current (2026-07-11) text specifies.
