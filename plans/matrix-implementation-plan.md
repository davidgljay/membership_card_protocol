...
- ~~**Before Phase 3, Step 10 (chain walk port):** confirm reading `plans/app-card-chain-walk-implementation-plan.md`... a subtly incorrect Python re-implementation of the chain walk is a security bug~~ — **superseded 2026-07-11.** Step 10 is no longer a from-scratch port; it integrates `membership-card-verifier`, an already-built, separately-maintained Python port of the verifier package, validated against shared cross-language interop vectors as its own responsibility. The remaining pre-Step-10 checkpoint is narrower: **confirm the chain-address-list open item (Step 10's write-up) has an explicit resolution — either the verifier package exposes `chain_card_addresses` on its public result types, or `chain_context.py` knowingly depends on its internal `stages.stage3` module** — before Step 10 is implemented, not discovered partway through.
...
### Step 9: Implement provider adapters and the predicate evaluator (rewritten 2026-07-11 — was "port the on-chain/IPFS read clients")

**Why this changed:** the original plan had this module hand-port an Arbitrum RPC client and an IPFS client from the existing TypeScript. A separate, already-built Python package — `membership-card-verifier` (`membership_card_verifier/packages/verifier-py`), a behaviorally-identical port of `@membership-card-protocol/verifier`, validated against shared cross-language interop vectors as part of its own test suite — makes that unnecessary for the actual verification/chain-walk logic. What this module still needs to write is much smaller: **adapters** implementing that package's two `Protocol` interfaces (`RpcProvider`, `IpfsProvider`) against this deployment's actual RPC/IPFS endpoints, plus the room-predicate-document evaluator (which the verifier package has no concept of — that's Matrix-specific). **Not yet published to PyPI as of 2026-07-11** — depend on it via a local path or git URL for now (mirroring exactly how `press/` referenced `@membership-card-protocol/verifier` via a `file:` path before its own npm publish), and switch to a versioned PyPI dependency once publish approval lands.

**What:** Three sub-steps, each independently testable.

- **9a. RPC provider adapter (`rpc_provider.py`).** Implement `membership_card_verifier.RpcProvider`'s async methods (`get_card_entry`, `is_policy_authorizer`, `get_press_authorization`, `get_sub_card_entry`, `get_log_entries`, `get_eas_annotations`) using `web3.py` against `ARBITRUM_RPC_URL`/`REGISTRY_CONTRACT_ADDRESS`, mirroring the same registry contract calls `wallet-service/src/config.ts` and its TypeScript registry-read call site already make (find and read that code first — do not guess the contract ABI or call shape; the verifier package's `RpcProvider` Protocol defines the method signatures precisely, but the actual contract-call implementation behind each one is this module's own work, same as it always was). **Unchanged from the original plan:** also exposes a WebSocket-subscription-capable interface (`arbitrum_rpc_ws_url`) for Step 11a's watcher to filter `CardHeadUpdated` events by address — the verifier package's `RpcProvider` has no concept of event subscriptions (it's a point-read interface only), so this half of 9a is still fully new code, not adapter-writing.

- **9b. IPFS provider adapter (`ipfs_provider.py`).** Implement `membership_card_verifier.IpfsProvider`'s single `async def fetch(cid: str) -> bytes` using `httpx` against `IPFS_GATEWAY_URL` — much smaller than the original from-scratch-client scope, since the verifier package handles everything past raw bytes (decryption, parsing, error classification). Must raise on an unresolvable CID (the package treats a raised exception as a provider failure, not folded into its own error/result types) — see the verifier package's README §Providers for the exact contract.

- **9c. Predicate evaluator (`predicates.py`).** A direct, careful port of `card_protocol_spec.md §The Predicate System`: `any_of`/`all_of`/`none_of` combinators, and leaf predicates `issued_under_template`, `chain_includes`, `card_field_matches`, `is_holder`, `is_issuer`, `chain_depth_at_most`, `code_equals` (note: `code_equals` is only meaningful in `revocation_permissions` context, not room policy — implement it but document that room policies should not use it, per the spec's own scoping note). **This still has to be written from scratch — the verifier package doesn't have a concept of room-predicate documents, that's this protocol's Matrix-specific addition.** Evaluated against the verifier package's real result types (`CardVerificationResult`/`SignatureVerificationResult`, plus `chain_context.py`'s output for the chain address list — see Step 10's open item) rather than an invented `ChainWalkResult` shape. Every leaf predicate needs a corresponding unit test with both a true and a false case.

**Who:** Sonnet for 9a (still needs to read and faithfully mirror the real contract-call shape) and 9b (smaller now, but still real adapter-writing work against the Protocol contract). Haiku is acceptable for 9c *only if* Step 1's spec plus the direct quote from `card_protocol_spec.md` lines 138–186 is provided verbatim as context — the predicate grammar is fully specified there and this becomes a mechanical translation.

**Data needed:** `wallet-service/src/config.ts`, the TypeScript registry-read call site (locate via search — likely near `wallet-service/src/chain/subcard-registry.ts`), `membership_card_verifier/packages/verifier-py/README.md` (the `RpcProvider`/`IpfsProvider` Protocol definitions and their exact method signatures), `card_protocol_spec.md` lines 138–186 verbatim.

**Done when:** All three modules have passing unit tests. The predicate evaluator's test suite covers every leaf predicate type and at least one nested combinator case (e.g. `all_of` containing an `any_of`). The two provider adapters are tested against a local mock/fixture, not live network calls — **for 9a's WS variant, test against a mock event stream, not a live subscription** — and each adapter's tests confirm it actually satisfies the `Protocol` it implements (e.g. via `isinstance` checks with `runtime_checkable`, or simply passing it to a real `CardVerifier` instance in a test and confirming it works end-to-end).
...
### Step 10: Integrate the verifier package for chain-walk/revocation data (rewritten 2026-07-11 — was "port the card chain walk")
...
**What this step actually is now:** implement `chain_context.py`, a thin integration wrapper: build a `VerifierConfig` (this module's `rpc_provider`/`ipfs_provider` instances, `app_certification_root`, `trusted_roots`, etc. from module config), then call `CardVerifier.verify_envelope()` (for a join attestation, which is already `SignedMessageEnvelope`-shaped per `matrix_join_attestation_and_revocation.md §1`) or `CardVerifier.verify_card()` (for a post-time re-check against a bare address, e.g. triggered by the watcher). Both return real, documented result types (`SignatureVerificationResult`/`CardVerificationResult`) — `revocation.status`, `chain_reaches_trusted_root`, `is_currently_valid`, etc. — rather than a bespoke `ChainWalkResult` this plan would otherwise have invented.

**Open item, discovered during this rewrite, not resolved by this document — must be resolved before Step 10 is actually implemented:** neither `SignatureVerificationResult` nor `CardVerificationResult` exposes the full **list of chain addresses** (`chain_card_addresses`) — only the boolean `chain_reaches_trusted_root`. The address list is computed internally by the verifier package's Stage 3 but never surfaced on its public result types. Two things this module needs specifically require that list: the `chain_includes`/`card_field_matches` predicates (which must check *every* card in the chain, per `card_protocol_spec.md §The Predicate System`, not just whether it reaches a root) and Step 11a's watch-set construction (which needs every address to subscribe to for `CardHeadUpdated`). Two ways to close this, neither chosen here:
- (a) Request the verifier package expose `chain_card_addresses` on its public result types — it's already computed by Stage 3 internally, so this is additive, not a redesign. Cleaner long-term, but requires a change to a package outside this plan's own scope, from whoever maintains it.
- (b) Have `chain_context.py` import `membership_card_verifier.stages.stage3` directly — works today, no upstream change needed, but leans on a module the package's own `__init__.py`/`__all__` doesn't list as public API, which could break without notice on a future verifier-package version bump.

**Who:** Sonnet — no longer "the highest-risk mechanical-translation step" (that risk moved to the verifier package's own test suite), but still requires real integration judgment: correctly constructing `VerifierConfig`, choosing `verify_envelope` vs. `verify_card` per call site, and resolving the chain-address-list open item above before this step can be called done.

**Data needed:** `membership_card_verifier/packages/verifier-py/README.md` (full — especially §Quick start, §Configuration, §Reading a result), the package's own source for `CardVerificationResult`/`SignatureVerificationResult`/`Stage3Result` (`src/membership_card_verifier/types.py`, `src/membership_card_verifier/stages/stage3.py`) to confirm exact current field names, Step 9a/9b outputs (the provider adapters this step's `VerifierConfig` is built from).

**Done when:** Given a set of fixture card chains (including at least one that should fail `chain_includes`, one that should pass, and one exercising `app_card` certification — reuse whatever fixtures `verifier-py`'s own test suite already has where possible, rather than re-authoring equivalents), `chain_context.py` produces correct predicate-evaluator inputs for each, **and the chain-address-list open item above has an explicit, documented resolution** (not silently defaulted to option (b) without saying so).

> **Checkpoint (revised 2026-07-11):** the original cross-check-against-TypeScript checkpoint no longer applies — that correctness burden now belongs to `membership-card-verifier`'s own test suite. What to actually confirm before marking this step done: (a) the chain-address-list open item above has a explicit, chosen resolution, not a silent default, and (b) `chain_context.py`'s fixture tests are drawn from (or cross-checked against) the verifier package's own test fixtures where they overlap, so this integration isn't tested in a vacuum disconnected from the package's own correctness guarantees.
...
- **Revised 2026-07-11:** does `chain_context.py` correctly integrate `membership-card-verifier` (not a from-scratch chain walk anymore), and does the chain-address-list open item (Step 10) have an explicit, documented resolution — not a silently-chosen default?
