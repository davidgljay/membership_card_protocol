...
## Current State (2026-07-16 — read this first if picking up fresh)

- **Phase 1 (spec documents): done.** All six spec documents committed (`13ebfa3c`) and internally consistent as of a dedicated milestone review (`plans/milestones/matrix-phase-1-summary.md`).
- **Phase 2 (infrastructure scaffolding): done and live-boot-validated.** Compose stack, Dockerfile, `homeserver.yaml.template` + render script, credential generation — all committed (`7edea566`). Actually booted (Docker daemon + real migrations + both generation scripts run) and confirmed Synapse parses its full config correctly, failing only at the expected point (the Phase 3 module package doesn't exist yet). Details: `plans/milestones/matrix-phase-2-summary.md`.
- **Phase 3 (Synapse policy module): done.** All of Steps 8–12a built and tested in `wallet-service/matrix-policy-module/` (73 passing tests). Two watcher-related checkpoints were reconfirmed 2026-07-12 (`watcher_backstop_interval_seconds=3600`, force-part-on-every-revocation with no 8xx/9xx distinction — both the recommended defaults). Full details, including a real deny-by-default gap found and fixed during the milestone review (unguarded predicate-evaluator exceptions) and three open items carried forward to Phase 4 (join-attestation wire transport, room-policy state-read API, Synapse Admin API force-part endpoint — each isolated behind a tested interface): `plans/milestones/matrix-phase-3-summary.md`.
- **Separate but load-bearing work: `membership-card-verifier`/`@membership-card-protocol/verifier` gained two new features** (`plans/verifier-criteria-and-chain-plan.md`, commits `2709e500`, `372d88fc`, `2258b908`): `return_chain` (exposes the full chain — addresses, pubkeys, decrypted card content — on `verify_envelope`/`verify_card` results) and `policy_match`/`conditions` (checks whether a chain includes a card issued under a given policy, with field-match conditions, exact-match or regex — exported as a standalone `evaluate_policy_match`/`evaluatePolicyMatch` function too, not just embedded in the result). Both packages: TS 101 tests, Python 130 tests (including 5 cross-language interop cases), `tsc`/`mypy` clean. This is why Phase 3's Step 9c ended up much smaller than originally planned — `predicates.py` is a thin loop over the exported function, not a general predicate-tree evaluator.
- **One remaining loose end, not yet resolved as of this note:** `membership-card-verifier` (PyPI) was still awaiting publish approval as of the last check — `matrix-policy-module/pyproject.toml` depends on it via a local `file:` path (`tool.hatch.metadata.allow-direct-references = true`) for now. Confirm current publish status before switching to a versioned dependency.
- **Phase 4 (Wallet-Service Bridge / Application Service): done.** All of Steps 13–16c built and tested (44 passing `wallet-service` tests + 12 passing `client-sdk` tests, plus 2 DB-integration tests blocked only by this environment's paused Docker/Postgres). Two of Phase 3's three carried-forward open items were resolved during this same 2026-07-12 session before Phase 4 started (join-attestation wire transport — rides in the `m.room.member` join event's content; force-part mechanism — in-process `ModuleApi.update_room_membership`, no admin token). The third (`RoomPolicyResolver`'s exact Synapse API) remains open, isolated behind a tested interface. One new gap surfaced and honestly flagged, not fixed: `wallet-service`'s server-side discovery endpoint (16c) has no real chain-walk `RpcProvider` wired yet — `matrix-policy-module/rpc_provider.py` (Python, Phase 3) already solved the identical problem and is the pattern to port. Full details: `plans/milestones/matrix-phase-4-summary.md`.
- **Phase 5 (Client-SDK Integration): done for `client-sdk`/`client-sdk-web`; `client-sdk-rn` is an honest scaffold, not a working implementation.** Steps 17–19 built and tested (297 `client-sdk` + 32 `client-sdk-web` + 28 `client-sdk-rn` passing tests). Step 17's checkpoint was resolved with David directly: web binding is `@matrix-org/matrix-sdk-crypto-wasm` (built and tested with a real `OlmMachine`, real Megolm round trip); React Native needs a custom Turbo Module against `matrix-sdk-crypto-ffi` (David's explicit "build," not "adopt `react-native-matrix-sdk`") — genuinely not buildable in this sandbox (no Rust/Xcode/Android toolchain), so `client-sdk-rn`'s crypto provider throws a descriptive "not implemented" error rather than faking success. Step 19's sender-binding check was verified against the exact worked-example attack from `matrix_encryption.md §4` (a validly-signed message from one card relayed under a different card's forged Matrix sender), not just a generic pass/fail. Full details, including an honest note on what wasn't testable (live-Synapse join-attestation rejection, real RN crypto): `plans/milestones/matrix-phase-5-summary.md`.
- **Phase 6 (End-to-End Testing and Documentation): done, with one honest scope gap.** Steps 20–22 executed against a real, live `docker compose up` stack (Docker became available this session) — not mocked. Booting the real stack surfaced nine genuine bugs invisible to unit tests alone, the most serious of which meant card-gating enforcement would have silently done nothing in a real deployment: `check_event_for_spam` (what `module.py` was registered against) is never invoked for joins in the installed Synapse version at all, and separately, `private_chat`'s preset-default `m.room.join_rules: invite` rejected every non-invited join via Synapse's own core event-authorization *before* any module callback could run. Both fixed — the real join gate is now `check_event_allowed` (`ThirdPartyEventRules`), and room creation now explicitly sets `join_rule: "public"`, making the policy module (not Matrix's invite system) the actual gate. `matrix-policy-module`: 86/86 tests passing. `wallet-service`'s Matrix-specific suites (room creation, discovery, live-stack lifecycle): 33/33 passing. Operator runbook written (`wallet-service/docs/matrix-operations.md`). **Scope gap, not a miss:** Step 22's on-chain-dependent smoke-test items (satisfying-card join, encrypted post/read, revocation force-part) need a deployed registry contract and funded test wallet, neither available in this sandbox — every item reachable without on-chain state passed. Full details: `plans/milestones/matrix-phase-6-summary.md`.
- **Next action:** The plan's phases are all complete except two explicitly-scoped-out gaps, neither blocking further sandbox work: (1) the RN native crypto module (Rust build pipeline + `uniffi-bindgen-react-native` + Turbo Module packaging — see `client-sdk-rn/src/MegolmCryptoProvider.ts`'s own header comment), and (2) Step 22's on-chain-dependent smoke tests, which need a deployed registry contract in an environment with a funded signer. Cross-server federation remains out of scope per this plan's locked-in decisions.
...
- Does the deny-by-default failure handling in `matrix_room_membership.md` §4 and `matrix_join_attestation_and_revocation.md` §3.3 together cover every external dependency the Synapse module and watcher have (RPC — both HTTP and WS, IPFS, and — corrected 2026-07-12, no admin API involved — the in-process `ModuleApi.update_room_membership` call and its power-level-dependent failure mode for force-part)?
...
### ~~Step 7b: Generate the watcher's Synapse admin-API token~~ (added 2026-07-11, **removed 2026-07-12 — no such token exists to generate**)

**Superseded 2026-07-12, during Phase 3 build-out.** Researched against current Synapse docs/source/issue tracker: there is no Synapse Admin API HTTP endpoint to force-remove a user from a room (the Room Membership admin API only forces a *join*; `element-hq/synapse#17885` asked for exactly this and was closed "not planned"). Force-part instead uses an in-process `ModuleApi.update_room_membership(sender, target, room_id, new_membership)` call (confirmed against current `synapse/module_api/__init__.py`) — a privileged in-process call available to the module directly, requiring no separate admin token at all. This step is dropped entirely; see the new Step 7d below for what its removal actually requires instead (an enforcement account with room power level, not a credential).

**What Step 7b's removal surfaces, not previously visible:** `update_room_membership` still enforces ordinary Matrix power-level auth on its `sender` parameter — the caller needs kick permission in that specific room, the same as any human moderator would. So a dedicated enforcement account (not a card-holder's own shadow account) must exist and be granted kick-level power in every card-gated room's initial `m.room.power_levels` state at creation time. This is a **new requirement on Step 16** (room creation, Phase 4, not yet built) that didn't exist under the (incorrect) HTTP-admin-token assumption — flag this explicitly when Step 16 is picked up.

### Step 7d: Provision the watcher's enforcement account (added 2026-07-12, replaces Step 7b)

**What:** Provision a dedicated Matrix account (e.g. `@matrix-policy-bot:<server_name>`) that `watcher.py`'s `ModuleApiForcePartClient` passes as `sender` to `update_room_membership` for every force-part call. This account needs no password/token of its own from the watcher's perspective (`ModuleApi.update_room_membership` is a privileged in-process call, not an authenticated HTTP request) — but it does need a real, `is_mine()`-recognized local user ID, and Step 16 must grant it kick-level power in every card-gated room it creates, or force-part calls will fail with a permission error at runtime. Store this account's Matrix user ID in the module's config (`enforcement_matrix_user_id` — add to `matrix_synapse_module.md`'s config schema) — no secret to generate or store via Step 7's pattern, since there's no credential here, just an identifier.

**Who:** Haiku — mechanical config/provisioning against a settled design; the account itself can be created the same way any other Application-Service-provisioned account is (Step 15b), just with a fixed, well-known user ID rather than one derived from a card_hash.

**Data needed:** This step's own text (the design is settled here), Step 15b's provisioning pattern (to reuse for account creation), Step 16's room creation flow (to confirm the power-level grant lands there).

**Done when:** The enforcement account exists on the homeserver; `matrix_synapse_module.md`'s config schema documents `enforcement_matrix_user_id`; Step 16's own done-when-criteria (once built) includes confirming this account received sufficient power level in a newly created room.

### Step 7c: Generate the membership registry's encryption key (added 2026-07-11, persistence decision)

**What:** The membership registry (Step 12a, `matrix_join_attestation_and_revocation.md §2a`) is encrypted at rest. Generate its encryption key using the **same secrets pattern established in Step 7** — this is a new credential, and the whole point of Steps 7/7c reusing one pattern is that nobody has to re-derive "is this stored safely" from scratch per credential.

**Who:** Sonnet.

**Data needed:** Step 7 output (pattern to reuse).

**Done when:** The key is generated and stored via the existing secrets abstraction; not committed to git; documented in the operator runbook (Step 21) alongside the other Matrix-specific credentials (AS token, membership registry key — **not** a watcher admin token, which doesn't exist).

---

### Phase 2 Milestone Review

**Context needed:** Steps 5–7 outputs (Step 7b no longer exists — see its removal note; this review predates that correction and its second bullet below is superseded).
...
- ~~Is Synapse's admin API reachable only from inside the Docker network...~~ **Superseded 2026-07-12 — moot.** There is no Synapse admin API involved in force-part; it's an in-process `ModuleApi` call. The equivalent scrutiny now belongs on Step 7d's enforcement account: confirm it's granted power only in card-gated rooms it needs to moderate, not given any broader standing privilege.
- Are all generated secrets (Synapse signing key, AS token pattern precedent, and the membership registry's encryption key, Step 7c) handled through the existing secrets abstraction, with nothing sensitive committed to git? (No watcher admin token — it doesn't exist.)
...
4. **Force-part:** removes the affected Matrix account from the room via an in-process `ModuleApi.update_room_membership(sender=<Step 7d's enforcement account>, target=matrix_user_id, room_id=room_id, new_membership="leave")` call — **corrected 2026-07-12: not Synapse's admin API**, which has no endpoint for this (see Step 7b's removal note). Applies uniformly to every detected revocation, 8xx or 9xx — no severity-based branching (see the 2026-07-11 decisions section at the top of this document). Requires the enforcement account (Step 7d) to hold kick-level power in the room, granted at room creation (Step 16) — a failure here (e.g. insufficient power level) falls into the same retry-with-backoff path as any other force-part failure.
...
**Data needed:** `specs/process_specs/matrix_join_attestation_and_revocation.md §3` (full), Step 7d output (enforcement account's Matrix user ID — not an admin token, which doesn't exist), Step 9a output (WS client), Step 10 output (visited-address list), Step 11 output (cache interface).
...
**What (amended 2026-07-11; amended again 2026-07-12):** Implement `wallet-service/server/routes/matrix/rooms/index.post.ts` per the shape defined in `specs/object_specs/matrix_room.md`: given `{ card_hash, policy_id, name?, topic? }` from an authenticated card holder, this endpoint (a) provisions the creator's shadow account if needed (Step 15b), (b) calls Synapse's Client-Server `/createRoom` API as that shadow account, with `preset: private_chat`, `m.room.encryption` initial state set to `m.megolm.v1.aes-sha2` (per `matrix_encryption.md`), an `m.card.policy` initial state event containing `{ policy_id }`, **and (new, 2026-07-12) an `m.room.power_levels` initial state event granting Step 7d's enforcement account (`enforcement_matrix_user_id`) at least the room's kick level** — without this, `ModuleApiForcePartClient.force_part` (watcher.py) will fail with a permission error on every revocation for this room, since `ModuleApi.update_room_membership` enforces normal Matrix power-level auth on its `sender` (see Step 11a's force-part note), (c) **appends `{ room_id, policy_id, created_at }` to the public room index** (`specs/process_specs/room_discovery.md §1` — this is new; the original step had no awareness the index exists), (d) returns `{ room_id }`.

**Who:** Sonnet.

**Data needed:** `specs/object_specs/matrix_room.md`, `specs/object_specs/matrix_encryption.md`, `specs/process_specs/room_discovery.md §1`, Step 15 output, Step 7d output (enforcement account's Matrix user ID to grant power to).

**Done when:** A test call creates a room, and a subsequent direct read via `synapse-postgres` (there is no Synapse Admin API for state inspection either beyond what's already documented — read room state directly) confirms the room has the correct `m.card.policy`, `m.room.encryption`, **and `m.room.power_levels` (enforcement account at kick level or above)** state events set, **and a subsequent `GET /matrix/room-index` call (Step 16a) includes the new room.** **New (2026-07-12):** a follow-up test simulates a revocation for a member of this room and confirms force-part actually succeeds (not just that the state event was set correctly) — this is the concrete proof the power-level grant works, not just that it was attempted.
...
- Are all AS secrets confirmed stored via the existing secrets backend? (No watcher admin token to check — Step 7b was removed 2026-07-12; confirm instead that Step 7d's enforcement account was granted power level in the room this phase's test creates.)
