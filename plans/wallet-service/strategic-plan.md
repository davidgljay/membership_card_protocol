...
- **UMBRAL re-encryption keys** (one per registered sub-card, OQ-WS-4 below): a stolen re-encryption key alone cannot decrypt anything — it can only re-encrypt to the specific sub-card public key it was issued for, and an attacker would still need that sub-card's private key (on-device) to do anything with the result. The residual risk is sub-card topology exposure (how many sub-cards a card has), which is already visible on the storage contract. **These are stored in plaintext** — encrypting them adds storage complexity without closing a real attack path. The actual risk in this area is a malicious sub-card key being registered at setup time, which is an authentication-layer concern, not a storage-layer one.
...
**OQ-WS-4: UMBRAL re-encryption key protection — RESOLVED**

Resolved: stored in plaintext. See "Secret Storage: Two Different Trust Levels" above — a stolen re-encryption key cannot decrypt anything without the corresponding sub-card private key, and the only exposure (sub-card topology) is already visible on the storage contract.
