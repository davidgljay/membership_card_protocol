...
The Phase 2.7 wallet-service audit found that `POST /cards/{card_hash}/subcards/{subcard_hash}/uuids` accepted a bare UUID array with no authentication: a caller who merely knew a `card_hash`/`subcard_hash` pair could register UUIDs and trigger message redelivery, with zero proof of controlling the sub-card's private key. This is a wallet-service issue, not a `relay-next/` issue, and is out of this phase's direct scope — but is recorded here per the audit finding.

**A separate subagent is actively fixing this concurrently, in the same repository, right now.** Checked directly before writing this summary rather than assumed:

- The implementation appears substantially present in the working tree: `wallet-service/src/auth/subcard-uuid-signature.ts` implements signed-envelope verification (resolve the sub-card's on-chain public key via the Arbitrum registry and IPFS, confirm `keccak256(pubkey) == subcard_hash`, verify an ML-DSA-44 signature over the canonicalized payload), and the route handler (`wallet-service/server/routes/cards/[card_hash]/subcards/[subcard_hash]/uuids.post.ts`) has been rewritten to call into it via a new `src/routes/subcard-uuid-registration.ts` module. Nonce/replay-protection infrastructure also exists (`server/db/subcard-uuid-nonces.ts`, a new DB migration, and a pruning task).
- **However:** that subagent's own task record states explicitly "No git commits (deferred to a consolidated pass)," and its task status is still `in_progress` at the time of writing. `git status` confirms this — every wallet-service file involved is modified or untracked, none committed. A test file (`wallet-service/test/subcard-uuid-signature.test.ts`) exists, but the route-level test the handler's own comment references (`subcard-uuid-registration.test.ts`) was not found, suggesting that layer may still be mid-flight.
- **Conclusion, stated plainly:** the fix is not yet done. Code that appears to implement it exists in the working tree but is uncommitted and the owning task is still in progress. This phase's own commits deliberately never touched any `wallet-service/` file, per the explicit instruction to avoid a mid-edit commit of someone else's unfinished work. Whether this gap is fully closed should be re-checked against that subagent's own completion report, not against this document.

### Redis Cloud staging validation

Still pending. `relay-next/PROVISIONING.md`'s checklist item for creating the primary Redis Cloud database is unchecked (the KV namespace item, by contrast, is checked — done 2026-07-02, binding `mcard_relay`). No real Redis Cloud credentials exist in this environment, so the storage layer (`redis/uuid-store.ts`, `message-store.ts`, `credential-store.ts`, `delete-queue.ts`, `reconciliation.ts`) has only ever been exercised against the hand-rolled RESP test server (`redis/test-harness.ts`, `test-resp-server.ts`) — genuine wire-protocol coverage, but not a real managed Redis Cloud instance. This remains a user action per the Phase 1 summary's same finding; nothing in Phase 2 changes that status.
...
2. User provisions the primary Redis Cloud database per `relay-next/PROVISIONING.md`'s remaining checklist items, then re-run the storage-layer tests against it (or at minimum the post-provisioning verification steps already listed there) before trusting the storage layer against anything beyond the hand-rolled test server.
