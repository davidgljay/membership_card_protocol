...
- New sibling directory `relay/` (not `relay-serverless` or similar — checked the monorepo for an existing parallel-service naming convention first; found none, so used the plan's suggested default).
...
Built in `relay/spike-do-ws/` as a standalone worker (see that directory's `README.md` for the full reasoning) rather than wired into the main Nitro route tree, because of two rough edges found while building it:
...
2. User decides on Redis Cloud plan/tier and provisions both databases per `relay/PROVISIONING.md`; the `CONFIG GET save`/`appendonly` verification in that checklist should be run before Phase 2 code is written against it.
