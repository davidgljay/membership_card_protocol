...
**Object spec:** [specs/object_specs/client_sdk.md](../../specs/object_specs/client_sdk.md)

> **Spec authority, going forward.** `specs/object_specs/client_sdk.md` is now the authoritative reference for this package's shape and behavior — both what's built and what's planned (its §14 tracks status per step). Before starting any step below, read the corresponding section(s) of that spec, not just this plan's own step description: where the two differ, the spec's documented interfaces, module layout, and invariants (§2, §12) take precedence, since it was written by consolidating the actual implemented code plus the remaining plan surface, and is meant to be kept current as new steps land. Any step that adds or changes public API shape, a provider interface, a security invariant, or an implementation status must update `client_sdk.md` in the same change — do not let it drift back out of sync with the code.
...
- Who: Claude
- Context needed: `specs/object_specs/client_sdk.md §9.2` (planned shape — update on completion), `specs/subcards.md §Sub-Card Request Flow Step 2`, `specs/object_specs/press.md §5.4 verifyAppCertificationChain` (the pattern being mirrored), `specs/object_specs/card_verifier.md §5` (`VerifierConfig.trustedRoots`, `fetchAnnotations`)
...
- Who: Claude
- Context needed: `specs/object_specs/client_sdk.md §9.3` (planned shape — update on completion), `specs/subcards.md §Sub-Card Request Flow Steps 3–4` (including the self-signing exception)
...
- Who: Claude
- Context needed: `specs/object_specs/client_sdk.md §9.4` (planned shape — registration and 8xx revocation only; §9.5 already documents the deregistration half, built ahead of schedule in Phase 2 — update §9.4's status on completion), `specs/subcards.md §Sub-Card Revocation, §Authorization for Deregistration`, `specs/process_specs/subcard_creation_policy.md`, `specs/process_specs/card_updates.md`, `specs/object_specs/press.md §5.4`, `plans/client-sdk/strategic-plan.md §Goal 7`
...
For a fresh agent starting any step, the minimum context to load. `specs/object_specs/client_sdk.md` is listed first in every row deliberately — read it before the phase-specific material, per the spec-authority note above.

| Phase | Minimum context |
|---|---|
| Phase 1 | `specs/object_specs/client_sdk.md §§1–6`, `plans/client-sdk/strategic-plan.md` (full), `specs/object_specs/card_verifier.md`, `membership_card_verifier/packages/verifier/*` (conventions to mirror), `specs/process_specs/oblivious_transport.md` (standalone spec for Steps 1.4a–1.4d — envelope format, key-config discovery, scope table, operator-separation caveat); for Step 1.4b specifically: `relay/server/utils/app-registry.ts`, `relay/server/api/deliver/[uuid].post.ts`; for Step 1.4c specifically: `wallet-service/src/secrets/*`, `wallet-service/server/routes/accounts/index.post.ts`, `wallet-service/server/utils/auth.ts`; for Step 1.4d specifically: `press/src/handlers/open-offer.ts`, `press/server/api/open-offer/claim.post.ts`, `press/src/config.ts` |
| Phase 2 | `specs/object_specs/client_sdk.md §7`, `plans/client-sdk/milestones/phase-1-summary.md`, `specs/process_specs/wallet_backup_and_recovery.md`, `plans/wallet-service/implementation-plan.md §Phase 2–3` |
| Phase 3 | `specs/object_specs/client_sdk.md §8`, `plans/client-sdk/milestones/phase-2-summary.md`, `specs/process_specs/card_offering_and_acceptance.md`, `specs/process_specs/open_offer_creation.md`, `specs/process_specs/open_offer_acceptance_new_wallet.md`, `specs/process_specs/open_offer_acceptance_existing_wallet.md`, `specs/object_specs/press.md §4, §5.1–5.2`, `plans/client-sdk/strategic-plan.md §Goal 7` (why press submission is oblivious-routed) |
| Phase 4 | `specs/object_specs/client_sdk.md §9` (§9.1 and §9.5 reflect what's already built; §9.2–9.4 describe the planned shape to implement against), `plans/client-sdk/milestones/phase-3-summary.md`, `specs/subcards.md`, `specs/process_specs/subcard_creation_policy.md`, `specs/process_specs/card_updates.md`, `specs/object_specs/press.md §5.4`, `plans/client-sdk/strategic-plan.md §Goal 7` |
| Phase 5 | `specs/object_specs/client_sdk.md §10` (planned shape), `plans/client-sdk/milestones/phase-4-summary.md`, `specs/messaging_protocol.md`, `specs/process_specs/message_routing.md`, `specs/process_specs/notification_relay.md`, `specs/object_specs/relay.md` |
| Phase 6 | `specs/object_specs/client_sdk.md §11` (planned shape), all previous milestone summaries, `plans/client-sdk/strategic-plan.md §All Goals` |
