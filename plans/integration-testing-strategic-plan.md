...
1. **Contract deployment in-stack** — *Resolved by inspection*: `contracts/scripts/deploy.sh` supports only sepolia/mainnet with an external RPC and funded key. The stack needs a `local` network case targeting the Nitro devnode's pre-funded dev key; handled in Phase 1. **Amended 2026-07-18 (David's decision):** the local-devnode path deploys contracts successfully but calling deployed Stylus contracts on it doesn't work (a genuine WASM-execution issue on `nitro-node`, unrelated to the protocol contracts — see `integration_tests/reports/phase-1-environment-notes.md`). Rather than keep digging, the stack now points at the existing Arbitrum Sepolia deployment (`contracts/deployments/sepolia.json`) for its chain component. The local-devnode work is kept (behind Compose profile `local-chain`, not deleted) to revisit later — the deployment-half of the fix (ArbOS version gate on multi-fragment Stylus contracts) is real and independently useful.
2. **workerd bindings** — *Resolved by inspection*: wallet-service has `wrangler.toml` (WALLET_KV; miniflare emulates KV under `wrangler dev`). Press has no wrangler.toml — one must be authored; its S3 access is via `@aws-sdk/client-s3`, emulated with MinIO. Press also reads `EXTERNAL_KV_URL`. **Amended 2026-07-18:** the MinIO-emulation half is superseded — press's IPFS pinning is now behind a pluggable `IpfsPinningProvider`, with a `kubo` implementation that talks to a real local Kubo node directly (no S3 shim needed). Set via `IPFS_PROVIDER=kubo` + `KUBO_API_URL`/`KUBO_GATEWAY_URL` (`press/src/config.ts`). The wrangler.toml-authoring and `EXTERNAL_KV_URL` parts of this answer are unaffected, still pending Phase 1 Step 1.6.
