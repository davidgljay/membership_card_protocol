...
/** Stub wallet-service (for the Phase 2 setupWallet call) + press, sharing one transport. */
...
describe('acceptOpenOfferForExistingWallet', () => {
  it('adds the new card to an existing wallet (from a real Phase 2 setupWallet call) without a new passkey or re-derived decryption_key', async () => {
    const { transport, state } = makeCombinedTransport();

    // --- Existing wallet state, from a real Phase 2 setupWallet call. ---
