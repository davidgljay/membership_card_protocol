...
import type { SubCardDocumentFields, WalletAppCardIdentity } from '../wallet/deviceSubCard.js';
 ...
 * Step 1`, `protocol-objects.md §16`) — the general, third-party-app
 * variant of what `wallet/deviceSubCard.ts` already implements for the
 * wallet's own "self-signing" case (Step 2.2). Reuses that module's
 * `SubCardDocumentFields`/`WalletAppCardIdentity` types directly rather
 * than redefining the identical protocol object and app-identity shape a
 * second time — `WalletAppCardIdentity`'s `{ cardPointer, publicKey, sign
 * }` shape is exactly "an app's own card identity + signing capability,"
 * which is what any requesting app (not just the wallet acting as its own
 * app) needs here too.
 *
