# Phase 1 Red-Team Report — Card Protocol v0.3
## Zero-Click Infrastructure Attacks

**Date:** 2026-05-22  
**Scope:** Steps 1.1–1.4 per `plans/implementation-plan.md`  
**Sources reviewed:** `specs/ARCHITECTURE.md`, `specs/card_protocol_spec.md`, `specs/protocol-objects.md`, `card-validator/src/`, `plans/strategic-plan.md`, `raw_notes/`
...
| 1.1 | 1.1-C | Serialization BINARY_FIELDS gaps in card-validator | Medium | Low | Low | Low |
...
| 1.2 | 1.2-B | Policy card unavailability cascades to all issued cards | **High** | High | Medium | Low |
...
| 1.3 | 1.3-B | State-actor traffic correlation: IP → card identity → site | **High** | High | Theoretical | Theoretical |
...
The Arbitrum One registry contract is the protocol's write gate. Every new card entry and every log-head update must be signed by a press sub-card key, verified on-chain via a Stylus (WASM) implementation of ML-DSA-44. The contract enforces that only presses listed in the governing policy's `approved_presses` may write to cards under that policy. ML-DSA-44 keys are 1,312 bytes; signatures are 2,420 bytes. Full on-chain verification was explicitly chosen over a hash-commitment shortcut.
...
The acceptance criterion states: "A press sub-card whose mutable pointer does not appear in `approved_presses` is rejected by the Arbitrum One registry contract." However, `approved_presses` is a `card-pointer-array` field inside the policy card document, which is stored on IPFS — not on-chain. A Stylus contract running in the EVM cannot fetch IPFS content.

The arcardectural description in `ARCHITECTURE.md` says the contract "verifies this by checking the press sub-card's own registry entry (which is on-chain) and confirming it is not revoked." This description covers only revocation status, not `approved_presses` membership. If this is the actual implementation, the write gate checks only that the signing press sub-card's on-chain entry exists and is not revoked — but does not verify that the policy authorizer has approved that press for the specific policy being written to. Under this implementation, any valid, non-revoked press sub-card could write registry entries for any policy, including policies whose authorizer has never approved that press.
...
2. **Calldata-provided `approved_presses` with CID verification (safe but expensive):** The press provides the full policy card content as calldata; the contract verifies the CID matches the on-chain head pointer for the policy, then checks `approved_presses`. Expensive in calldata terms but correct.
3. **Revocation-only check (unsafe):** The contract only verifies that the press sub-card is not revoked on-chain, without checking `approved_presses` membership. This is the interpretation supported by the current spec text.

**If implementation option 3 is deployed:** any party who obtains any valid, non-revoked press sub-card — even one authorized for a different policy or issued for a test deployment — can write arbitrary log entries to any card in the registry. This includes posting fraudulent 9xx revocations against cards in policies that never authorized the attacker's press.
...
### Finding 1.1-C — Serialization BINARY_FIELDS Gaps in card-validator
...
The `card-validator/src/serialization.ts` BINARY_FIELDS set is missing several card-pointer fields that Appendix A of the spec requires to be encoded as CBOR byte strings rather than text strings. Missing fields:

- `target_card` — present in `UpdateIntentPayload`, signed by the updater
- `updater_card` — present in `UpdateIntentPayload`, signed by the updater
- `requester_card` — present in `AuthenticationRequest`, signed by the requester
- `card_pointer` — present in `AuthenticationResponse`

When the validator canonicalizes an `UpdateIntentPayload` for intent signature verification, `target_card` and `updater_card` are encoded as CBOR text strings (major type 3) instead of byte strings (major type 2). This means:
...
**Mitigation:** Add `target_card`, `updater_card`, `requester_card`, and `card_pointer` to `BINARY_FIELDS` in `serialization.ts`. Add conformance test cases for `UpdateIntentPayload` and `AuthenticationRequest` to the corpus.
...
The on-chain registry holds only the current log head CID per card. All content — the genesis CardDocument, every LogEntry, the policy card, and the press issuance log — lives on IPFS and persists only while someone is pinning it. Presses are contractually responsible for pinning but the protocol has no on-chain mechanism to verify pinning status or to prevent targeted de-pinning. The `card-validator` fetches IPFS content in `getAllLogEntries()` and propagates fetch failures as errors.
...
A press operator (or anyone who has compromised or coerced the press's pinning infrastructure) can stop pinning a specific holder's card log. The on-chain head CID pointer remains intact, but when a verifier calls `getAllLogEntries()`, the IPFS fetch fails. The validator propagates this as "Chain walk failed" — the card is unverifiable, not definitively valid or revoked.
...
- **Targeted**: The press can de-pin specific holders' card logs while leaving others intact, enabling selective credential invalidation.
...
- *State actor*: Compels the press (or the press's IPFS pinning service) to stop pinning cards associated with specific activists or journalists. Targeted, deniable, leaves no formal revocation record.
...
3. A protocol-level "content freshness" check — verifiers that receive IPFS timeouts should flag the card as unverifiable-pending rather than invalid, and retry from multiple gateways before concluding unavailability.

---

### Finding 1.2-B — Policy Card Unavailability Cascades to All Issued Cards

**Severity:** High  
**Feasibility:** Practical (same pinning control as 1.2-A)  
**Adversary relevance:** State actor (High), Criminal org (Medium), Individual abuser (Low)

If the policy card's IPFS content is de-pinned or becomes unavailable, the consequences are broader than de-pinning an individual card:

- Verifiers cannot evaluate the `field_definitions` to confirm a card's fields are policy-compliant.
...
- Auditors cannot read the press issuance log (which is anchored to the policy card's log).
- Any card issued under the policy is now unverifiable for policy-compliance purposes, even if the card's own log is still pinned.

The on-chain head CID for the policy still exists, so the policy is not formally revoked. But any service that performs full verification (including policy compliance) will reject all cards under the policy.

This is effectively a community-wide credential disruption achievable by targeting a single IPFS entry (the policy card content), rather than de-pinning each holder's card individually.

**Adversary application by tier:**
- *State actor*: Targeting a community's policy card content disrupts all credentials for that community simultaneously, without any formal action that could be legally contested.
- *Criminal org*: Less motivated, but could disrupt a competitor's trust ecosystem.

**Mitigation options:**
1. Require that policy card content be replicated across at least N IPFS nodes before the Arbitrum One registry pointer is updated (OQ-3 in ARCHITECTURE.md is the open question for this threshold).
2. Filecoin archival for policy cards specifically should be treated as mandatory for any policy with real-world consequences.
3. Client-side caching of policy card content, with explicit version tracking to detect when cached content has gone stale.
...
Because IPFS content is content-addressed, the attacker cannot serve fake content at the correct CID — a hash mismatch would be detected. They can only refuse to serve the correct content. The result is "verification failure" at the revoked entry, not "card appears valid."

This does not make the revoked card appear valid to a correct implementation. It causes `chain walk failed` — not a clean validation pass. However, a permissive verifier that treats chain-walk failures as non-blocking would effectively be fooled.

**Distinction from Finding 1.2-A:** Finding 1.2-A (de-pinning before a 9xx) makes a non-revoked card unverifiable. Finding 1.2-C (de-pinning the 9xx itself) makes a revoked card appear unverifiable rather than revoked. The practical difference depends on verifier policy: conservative implementations reject both; permissive ones may pass both.

**Mitigation:** Verifier implementations must treat chain-walk failures as equivalent to revocation, not as verification passes. The `card-validator` currently propagates chain-walk failures as errors (not as "valid"), which is the correct behavior. Services consuming the validator must treat error responses as denial-of-authentication.
...
Each card's metadata includes a Nym gateway address. Senders route encrypted payloads through the Nym mixnet to this gateway. For "fully public" cards the gateway address is plaintext; for private cards it is visible only to capability bundle holders. The authentication flow prefers Nym > OHTTP > HTTPS for response delivery. The gateway address is the Nym-side endpoint, held by the message server; the message server can observe when messages arrive but not their content.
...
The Nym gateway address is a real-time network endpoint. For public cards, it is plaintext in card metadata and enumerable by any verifier. An attacker who knows the gateway address can flood it with traffic, overwhelming the gateway and preventing legitimate message delivery.
...
**Adversary application by tier:**
- *State actor*: Can target specific known gateway addresses. For a journalist or activist with a public card, the gateway address is discoverable. A targeted DoS forces the HTTPS fallback, revealing the wallet service IP. Combined with §1.3-B correlation techniques, the IP links card identity to physical location.
...
### Finding 1.3-B — State-Actor Traffic Correlation: IP → Card Identity → Authentication Context
...
{
  "session_id": "...",
  "signed_statement": { ... "card_pointer": "..." },
  "card_pointer": "<holder's card identity>"
}
```

A de-anonymized authentication response links:
- **Physical IP address** → device and approximate geographic location
- **Card pointer** → the holder's community membership identity
- **Requester's card** → the site being authenticated to
...
2. Encourage holders serving high-risk populations to use separate cards per service (as noted in the authentication spec's unlinkability section), limiting correlation to individual service pairs rather than cross-service activity patterns.
...
The Nym gateway address is a stable field in card metadata — for public cards, it is plaintext and discoverable by anyone. Even without de-anonymizing Nym traffic, an observer who knows a card's gateway address can learn:
...
- **Liveness**: A gateway that receives no traffic may indicate the holder's card is inactive; a surge indicates renewed activity.
...
**Additional concern — cross-card correlation via shared gateway:** If a holder uses the same Nym gateway address for multiple cards (across different policies or communities), an observer who knows two of the holder's cards can confirm they share a gateway, linking otherwise pseudonymous identities.
...
2. Allow different cards to use different gateway addresses, decoupling community identities at the transport level.
3. The message server arcardecture (UMBRAL proxy re-encryption) already supports per-sub-card queuing; different cards routing to different gateways is arcardecturally feasible.
...
| Card pointer | Community membership; which credential was used |
| Requester's card | Which specific service/platform was authenticated to |
| Timestamp | When the authentication occurred |
...
- The `card_pointer` exposed in authentication responses is the same identifier used as the account ID on the requesting site, so the attacker can potentially match it to service-level account records if they also have access to the service.

The authentication response is designed to reveal the card pointer to the requester (that's how authentication works). The privacy protection is in the transport — Nym is supposed to prevent the requester from learning the wallet service's IP. De-anonymizing the transport breaks this protection completely.
...
The press is a networked service holding a funded Arbitrum One wallet, a press sub-card key, and a Nym-capable submission endpoint. It is the protocol's write intermediary: all registry writes, log updates, and revocations flow through it. The press encrypts audit log entries to auditor keys (ML-KEM) and cannot decrypt them. A press's sub-card pointer must appear in `approved_presses` for its writes to be accepted.
...
An attacker with the press's sub-card private key inherits write authority to the Arbitrum One registry for all cards under the press's authorized policies. The capabilities and their impact:
...
2. **Register new card entries for attacker-controlled keypairs**: The press constructs a CardDocument for a new card, signs it as the press, and generates an attacker-controlled holder keypair to provide the countersignature. The resulting card has a valid press signature and a valid holder countersignature — both from the attacker. Verifiers checking the card would see valid signatures, but the "holder" keypair has no real identity backing. This allows issuing fake credentials under compromised policies.
...
**Time-to-detection:** A holder whose card has been silently revoked with `notify_holder: false` learns about it only when:
...
3. Verifier-side "last checked" freshness indicators: relying parties that see a card should record the log-head CID at last verification; a change triggers re-verification before the next authentication is accepted.
...
| Which card pointers submit update intents | Identity of active credential holders; who is aware of their credential status |
| IP addresses of HTTPS submissions | Direct identification of submitters who don't use Nym |
| Nym gateway addresses from Nym-routed submissions | The Nym endpoint associated with each submitting card identity |
...
1. Knows the gateway address associated with each submitting card identity.
...
- New card issuance (no new cards can be issued while the press is down).
- All update intents, including 810 self-revocations.
- An 810 self-revocation (code 810: "this card's signing key compromised") cannot be submitted if the only approved press is unavailable or has been seized. The holder cannot post the revocation via the paymaster path either, because OQ-4 ("should the protocol support direct writes from the holder via paymaster?") is unresolved in v1.
...
- Refusing issuance requests from specific requesters or recipients: Blocks certain people from obtaining cards without any visible denial — the press simply does not respond.
...
> "If any Phase 1 step surfaces a Critical finding — an attack that appears to allow arbitrary writes to the registry, mass revocation of cards, or complete suppression of revocation records — pause and notify the author before continuing."
...
- **Finding 1.4-D** (selective censorship of 810 self-revocations) is closely related to the Phase 2, Step 2.3 scenario of sub-card key compromise combined with holder revocation attempt.
