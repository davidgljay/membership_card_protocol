/**
 * `specs/process_specs/card_updates.md` end-to-end, against the live dev
 * deployment. Ported from
 * integration_tests/suites/core/card_updates.spec.ts unchanged in test
 * logic -- only import sources changed (published `app-sdk`/`verifier`,
 * `mintLiveCard`/`PRESS_BASE_URL` from `../../support/liveCard.js`).
 *
 * Covers the post-issuance update flow from updater intent assembly through
 * press validation, log entry posting, and on-chain registry update:
 ...
 *
 * Known limitations, carried over from the original suite unchanged:
 *  - `ancestry_pubkeys` for freshly-minted test cards may point to ancestors
 *    not on-chain, causing full chain-of-trust walks to fail. Tests
 *    requiring chain resolution are marked it.todo with clear explanations.
...
describe('card_updates.md (live dev deployment)', () => {
  ...
  it.todo(
    'Phase 3-4: submits update intent to press and receives LogEntry confirmation (blocked by ancestry_pubkeys chain-of-trust — test cards\' ancestors not on-chain)'
  );
  ...
  it('Phase 3: rejects update attempt on immutable field (policy_id)', async () => {
    ...
    expect(updateRes.status).toBe(400);
    const error = (await updateRes.json()) as { error: string; message: string };
    expect(error.error).toMatch(/P-[0-9]+/);
  });

  it('Phase 3: rejects update intent on non-existent target card', async () => {
    const timestamp = new Date().toISOString();
    const updaterAddress = appSdkKeccak256(updater.publicKey);
    const fakeTargetAddress = '0x' + 'a'.repeat(40);
