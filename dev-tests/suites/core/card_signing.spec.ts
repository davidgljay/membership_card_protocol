/**
 * `specs/process_specs/card_signing.md` end-to-end, against the live dev
 * deployment. Ported from
 * integration_tests/suites/core/card_signing.spec.ts (Phase 3 Step 3.1's
 * pattern-setter) unchanged in test logic -- only the import sources
 * changed: `@membership-card-protocol/app-sdk` and `.../verifier` resolve
 * from node_modules here (installed at the `next` dist-tag), not a
 * monorepo `file:` path, and `mintLiveCard` comes from
 * `../../support/liveCard.js` (no governance bootstrap -- see that file's
 * doc comment). See dev-tests/README.md for the "Dev governance
 * prerequisite" this suite's `beforeAll` depends on.
 *
 ...
 * beyond minting the two identities used as real signers (spec
 * Postconditions: "any party ... can verify ... without a network call").
 * What makes this an *integration* test rather than a duplicate of
 * app-sdk's own unit tests is:
 *
 ...
 *     the two *published* packages' vendored crypto stays byte-identical
 *     as real npm artifacts, not just in source.
 *
 * Known gap carried over from the original suite (not this suite's to fix):
 * app-sdk's `MessageType` union doesn't yet cover the full `card_signing.md`
 * message type taxonomy -- tests below use `text`/`edit`, which are
 * implemented, to exercise co-signing/edit/retract/forward mechanics
...
describe('card_signing.md (live dev deployment)', () => {
  ...
  beforeAll(async () => {
    ...
    // mints can race it into a "nonce too low" tx failure (see the original
    // suite's identical note; vitest.config.ts's fileParallelism: false
  ...
  it('Phase 1-3: assembles, canonically serializes, and signs a payload', async () => {
    const payload = buildMessagePayload({
      type: 'text',
      content: { body: 'hello from the card_signing dev-tests suite', format: 'plain' },
  ...
  it('Forwarding: a ForwardPackage unambiguously identifies original sender, forwarder, and new recipients', async () => {
    ...
    expect(originalEntry.public_key).toBe(bytesToBase64Url(signer.publicKey));
    expect(forwardEntry.public_key).toBe(bytesToBase64Url(cosigner.publicKey));
    expect(forwardPackage.forward_envelope.payload.recipients).toEqual([thirdPartyRecipient]);
  }, 30_000);
