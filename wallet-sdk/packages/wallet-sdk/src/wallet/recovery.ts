 ...
 * and post-recovery sub-card deregistration
 * (`subcards.md §Deregistration After Key Recovery`).
 *
 ...
 *
 * The batch deregistration (`subCardDeregistration.ts`) is folded into this
 * same function, right after the master key is recovered and before
 * re-registration, rather than exposed as a separately callable step: it
 * needs `masterSecretKey` (the only valid signer,
...
export interface RecoverWalletOptions {
  ...
  /** The shared `CardVerifier` instance — see `deviceSubCard.ts`'s doc for why it's needed for device sub-card re-registration. */
  ...
  subCardKeyId?: string;
  /**
   * Sub-cards active before the simulated loss (`subcards.md §Deregistration
   * After Key Recovery`) — supplied by the caller (e.g. its own cached card
   * list), since neither the recovered keyring nor anything else this SDK
   * persists tracks sub-card issuance. Omit to skip batch deregistration
   * entirely.
   */
...
export interface RecoverWalletResult {
  ...
  /** Per-sub-card outcomes from batch deregistration; present only if `previouslyActiveSubCards` was supplied. */
...
export async function recoverWallet(options: RecoverWalletOptions): Promise<RecoverWalletResult> {
  ...
  try {
    // --- Batch-deregister sub-cards active before the loss, signed by the
    // freshly recovered master key — before re-registration, matching
    // `subcards.md`'s own ordering ("the holder should deregister all
    // existing sub-cards" as the first post-recovery action, ahead of "each
    // app should be prompted to re-request"). Skipped entirely if the
