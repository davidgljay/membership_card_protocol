import { canonicalize, keccak256, mlDsa44Verify } from '@membership-card-protocol/app-sdk';
 ...
 *
 * Fully owned by this package — per `plans/sdk-split-strategic-plan.md`'s
 * capability table ("Offer review + countersign + acceptance (recipient
 * side)" → Wallet SDK) and `wallet_sdk.md §7.1`.
 *
 * Judgment calls (this step's scope is verification, not chain-resolution
 * machinery — `CardVerifier`'s public surface is the only chain-walking/
 * revocation-checking primitive this SDK is allowed to use, and it doesn't
