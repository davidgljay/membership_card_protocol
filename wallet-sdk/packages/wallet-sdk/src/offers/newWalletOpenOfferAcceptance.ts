...
import type { SignedOpenCardOffer, Scip } from '@membership-card-protocol/app-sdk';
 ...
 *
 * Phases 1 (offer review), 2 (wallet setup), and 3 (countersign + claim
 * submission) are chained here. Phase 2 is "invoked inline" via
 * `setupWallet`'s `postSetupHook` (see that option's doc) rather than by
 * calling `setupWallet` as an opaque black box and separately re-deriving
 * `decryption_key` afterward — `setupWallet` never exposes it, by design,
 * and this flow doesn't need a new way to re-derive it: the hook runs the
 * claim countersigning while it's still valid, in the same
 * function-scoped lifetime `setupWallet` itself already establishes.
 *
