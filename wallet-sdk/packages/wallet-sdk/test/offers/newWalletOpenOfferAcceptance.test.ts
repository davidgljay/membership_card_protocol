...
function makeHappyPathRpc(issuerAddress: string, extraTrusted: string[] = []): RpcProvider {
  const trusted = new Set([issuerAddress, ...extraTrusted]);
  return {
    getCardEntry: async () => ({ log_head_cid: 'cid', policy_address: POLICY_ADDRESS, last_press_address: PRESS_CARD, forward_to: null, exists: true }),
    isPolicyAuthorizer: async (address) => trusted.has(address),
...
describe('acceptOpenOfferForNewWallet', () => {
  it('end-to-end: reviews the offer, sets up a new wallet, countersigns, and submits the claim to the press via ObliviousProtocolTransport', async () => {
    ...
    const cardVerifierForDeviceSubCard = makeAlwaysTrustingCardVerifier();
    ...
    const result = await acceptOpenOfferForNewWallet({
      ...
      walletAppCard,
      registerSubCard,
      cardVerifier: cardVerifierForDeviceSubCard,
