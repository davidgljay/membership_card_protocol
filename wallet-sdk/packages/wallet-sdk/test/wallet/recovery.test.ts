...
function makeStubWalletService() {
  ...
  const transport: ObliviousProtocolTransport = {
    request: vi.fn(async (destination: ObliviousDestination, requestOptions: RequestOptions) => {
      ...
      // A stub press — a distinct destination kind sharing this same
      // transport, exactly as recoverWallet's own deregistration step
      // calls it.
      ...
      if (method === 'PUT' && keyringUpdateMatch) {
        ...
        // `rotate_service_secret` (default true): `false` installs the blob
        // without minting a mismatched new secret — see
        // `wallet/recovery.ts`/`setupWallet.ts`'s doc comments and
        // `wallet-service`'s `keyring.put.ts` for why.
...
describe('recovery', () => {
  ...
  it('end-to-end: setup → simulated device loss → recovery initiation → window expiry → key release → keyring fetch from a stub non-primary instance → decrypt → re-registration, with the recovered keyring matching the original bit-for-bit before re-encryption', async () => {
    const { result: originalSetup, transport, state, forceExpire, secureKeyProvider: originalSecureKeyProvider, cardVerifier } =
      await performSetup();
    ...
    const wrappingKeyForAssertion = passkeyOutputFromPrf(syncedPasskeyPrfOutput);
  ...
  it('YubiKey recovery path: unwraps decryption_key using the provider-derived wrapping key', async () => {
    ...
    const { result: originalSetup, transport, state, forceExpire, cardVerifier } = await performSetup({
      yubiKeyProvider,
      yubiKeyPin: '1234',
    });
  ...
  it('throws if the recovered keyring has no entry for the given card_hash', async () => {
    const { result: originalSetup, transport, cardVerifier } = await performSetup();
  ...
  describe('post-recovery sub-card deregistration batch', () => {
    ...
    it('omits subCardDeregistrations entirely when no previously-active sub-cards are supplied', async () => {
      const { result: originalSetup, transport, forceExpire, cardVerifier } = await performSetup();
