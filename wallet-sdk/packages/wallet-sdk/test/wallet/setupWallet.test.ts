...
/**
 * A fake wallet app card identity. `cardPointer` is set to
 * `keccak256(publicKey)` — required now that `registerDeviceSubCard` routes
 * through `handleSubCardRequest`'s binding check, unlike the old parallel
 * self-signing path which never needed `app_card`/`app_card_pubkey` to be
 * self-consistent.
 */
...
/** A fake test registry standing in for the real press-submission flow. */
...
/** A CardVerifier that trusts any address it's asked about — the device sub-card's self-request always validates. */
...
function makeFixtures(fixedServiceSecret: Uint8Array, attestationObject: Uint8Array, credentialId: Uint8Array) {
  ...
  return { passkeyProvider, storageProvider, secureKeyProvider, walletAppCard, registerSubCard, cardVerifier, transport, state };
}
...
describe('setupWallet', () => {
  it('drives full setup against a stubbed wallet service and writes the keyring to StorageProvider', async () => {
    ...
    const { passkeyProvider, storageProvider, secureKeyProvider, walletAppCard, registerSubCard, cardVerifier, transport, state } =
  ...
  it('never persists service_secret in plaintext beyond the derivation step', async () => {
    ...
    const { passkeyProvider, storageProvider, secureKeyProvider, walletAppCard, registerSubCard, cardVerifier, transport } =
      makeFixtures(fixedServiceSecret, attestationObject, credentialId);
  ...
  describe('device sub-card generation and registration', () => {
  ...
  describe('backup registration', () => {
    it('always registers a synced-passkey backup, Bearer-authenticated, whose wrapped blob unwraps to decryption_key', async () => {
      ...
      // own bytes.
      const decryptionKey = deriveDecryptionKey(passkeyOutputFromPrf(attestationObject), fixedServiceSecret);
      ...
      // coincidental.
      const syncedPasskeyOutput = passkeyOutputFromPrf(DEFAULT_SYNCED_PASSKEY_PRF_OUTPUT);
