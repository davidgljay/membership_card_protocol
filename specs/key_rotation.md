...
4. **Retire the old sub-card's delivery queue.** Senders resolve the recipient's current sub-card list from the storage contract at send time (`process_specs/message_routing.md §Sender-Side Fan-out` — no wallet-held re-encryption keys to update, as of v0.4), so once the new sub-card is registered on-chain, new messages are naturally encrypted to it instead. The old sub-card's queue is flushed; the message server stops accepting delivery to the old sub-card.
