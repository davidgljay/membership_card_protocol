...
**Amends:** v0.7 — §1 records a temporary, explicitly-flagged pre-deployment exception: Upstash is in use for `REDIS_PRIMARY_URL` during testing because Redis Cloud's free tier has no TLS support at any level (confirmed by direct testing) and `relay`'s client hardcodes TLS as mandatory, while Upstash supports TLS on its free tier. This does not change the design — Upstash cannot satisfy the no-persistence invariant at any tier, so this is a knowing, temporary trade-off for non-real test data only, with a required cutover to a paid Redis Cloud tier before production. See `relay/PROVISIONING.md` §1a/§1b.
...
- The in-process `Map`-based WS/SSE connection tracking
  (`relay-old/src/routes/ws.ts`'s `activePeers`, `relay-old/src/utils/sse_connections.ts`)
...
WebSocket to a deployed Durable Object (`relay/spike-do-ws/`,
...
**Corrected during Phase 1 review — this subsection previously said `messages:{push_token}`, which contradicted §8.4 (Device Credential Store) later in this same document, and contradicted the actual behavior of the current implementation (`relay-old/src/utils/storage/redis.ts`'s `storeMessage(credential, ...)`). Push-token-keying would also have broken the isolation guarantee §8.1 claims — see that section's threat model. The key is, and has always been in the running code, `device_credential`-keyed.**
...
`relay/PROVISIONING.md` for the provisioning steps that produce
