...
Implementation-plan Step 4.3, not yet built. Will assemble a typed consent data structure on successful §9.2 validation — app identity (name/version/publisher, resolved from the decrypted app card), requested vs. grantable `capabilities` (the wallet may grant a subset, never more), advisory annotation warnings (none, until OQ-SDK-11 is revisited), and a default/suggested `valid_until`. A separate `countersignSubCardRequest(consentDecision)` call, invoked after the host app collects user approval (skipped entirely for the wallet's own self-signing exception, §7.4), will perform `holder_signature` and the IPFS post.
