...
**Date:** 2026-05-17  
**Author:** David Jay  
**Protocol dependency:** Chitt Protocol (npm package, separate codebase and brand)
...
1. **Verified exchange in under 10 minutes.** A community member with a Chitt can post an offer and have it accepted — with both parties' membership verified and a record created — without leaving the web browser or touching any blockchain tooling directly.
...
5. **Protocol adoption signal.** At least 3 communities in the first cohort use [Product Name] in ways that surface novel use cases or integration needs for the underlying Chitt Protocol, informing its roadmap.
...
- As an Organizer, I want to invite members by sending them a Chitt invitation link via WhatsApp so that they can join without creating a separate account.
...
- As an Organizer, I want to revoke a member's Chitt if they violate community trust so that accountability is enforceable and the community feels safe.
...
- As a Steward, I want to send Chitt invitation links to prospective members so that I can grow the community without requiring the Organizer to manage every invitation.
...
### Member
- As a Member, I want to receive an invitation link via WhatsApp and complete onboarding in under 5 minutes so that getting a Chitt does not feel like a separate project.
...
**Chitt issuance via invitation link**
- A member receives a URL (delivered via WhatsApp or any channel) that initiates Chitt onboarding
- Onboarding creates a keyring and keypair using the Chitt Protocol npm package; the user never sees raw key material
- The completed Chitt is registered and the member can access the offer board immediately after
- *Acceptance criteria:* A non-technical user can complete onboarding on mobile in under 5 minutes with no prior knowledge of the protocol; the resulting Chitt is verifiable via the Chitt Protocol

**Offer creation**
- A member with a valid Chitt can create an offer with: title, description, category (skill / good / time / financial ask), and optional expiry date
- The offer is signed with the member's Chitt at creation time
- Offers are visible on the community offer board
- *Acceptance criteria:* The signed offer can be independently verified via the Chitt Protocol; category filter works on the board; offers without expiry remain active until manually closed
...
- Clicking the badge shows the Chitt's chain summary: who issued it, when, and its current status
- *Acceptance criteria:* A revoked Chitt causes the offer to display a clear "membership not in good standing" warning; chain summary is accurate and fetched in real time

**Organizer dashboard**
- Organizer can view all members and their Chitt status (active / revoked)
- Organizer can generate and send invitation links to new members
- Organizer can revoke a member's Chitt with a reason (stored in the revocation log)
...
- The log shows: offer title, category, offerer Chitt (anonymized by default, expandable), acceptor Chitt, date
...
**Member profile page**  
Each member has a minimal profile page showing their active offers and (optionally) their exchange history. Linked to their Chitt pointer, not a separate account.
...
A financial ask offer type requires Steward approval before appearing on the board. When a Steward with appropriate Chitt scope approves, the ask goes live. Approval is logged in the exchange record. The Steward does not handle money — they confirm the ask is legitimate and the requestor is a verified member.

---

### P2 — Future Considerations (design should not foreclose these)

**Multi-community Chitt aggregation**  
A member holds Chitts from multiple communities and can access offers from all of them in a single interface. Privacy controls allow members to choose which communities can see their offers and acceptances.

**Annotation layer**  
Either party in an exchange can file a Chitt-signed report with community Stewards if the exchange goes poorly. Reports are not public by default; they are visible only to Stewards and the Organizer. A pattern of reports against a member can trigger a Steward review.

**Community federation**  
Two communities can establish a trust relationship so their members can access each other's offer boards. Governed by Organizer-level Chitt signing on both sides.

**Matrix room integration**  
A community can provision a Chitt-gated Matrix room alongside the offer board. Members use their Chitt to access the room. Useful for communities that want to migrate their WhatsApp group to a more federated communication platform over time.

**Mobile-native onboarding**  
A lightweight mobile app or PWA that stores sub-Chitt keys in the device's secure enclave, removing the need to manage a keyring manually. Improves the security posture for members who are sensitive to key management.
...
| Member Chitt activation rate | >40% of invited members create Chitt within 7 days of invitation | Chitt issuance log vs. invitation log |
...
| Exchange depth | Average member completes ≥2 exchanges (offered or accepted) | Exchange record per Chitt |
...
|----------|-------|-----------|
| Product name | David | Yes — needed before any external-facing work |
