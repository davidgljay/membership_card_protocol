...
**Changelog (spec-consistency Phase 1):** Fix #16 — corrected stale `wallet_ws_url` field name to `wallet_base_url`; Fix #17 — corrected Privacy Properties table and Process 2's lookup key from `push_token` to `device_credential`, matching `relay_data_model.md`'s isolation guarantee; Decision B — clarified that `ObliviousProtocolTransport` alone satisfies the anonymizing-transport requirement for `registerCardUuids`. See `plans/spec-consistency/inconsistencies/phase-1-consolidated-fixes.md`.

**Changelog (spec-consistency Phase 2):** Fix #54 — corrected three stale `(Process 5)` cross-references (Process 2 step 3, Process 2 step 4's SSE-open branch, Process 5 step 7) to `(Process 6)`, matching Process 6's renumbering after Process 4 (Device-Level SSE) was inserted. Fix #55 — expanded §"UUID Pools and Device Credential" to state the device credential authenticates `GET /sse`, `GET /pending`, **and** `POST /ack` (previously omitted `POST /ack`). Decision (e) — corrected the Failure Handling table's "Push token rotated by platform" row: push-token rotation is handled by replenishment-in-place (`POST /register` with the existing device credential updates `push_token` on the existing credential record) per `relay.md §6.3` and `relay_data_model.md §8.3`, not by issuing a new device credential or fresh UUIDs. See `plans/spec-consistency/inconsistencies/phase-2-consolidated-fixes.md`.
...
| Relay service | UUID → device credential + push token; device credential → pending message blobs | Card hash, card identity, subcard identity, message content (blobs are E2E encrypted) |
...
In addition to the UUID pool, each registration call returns a **device credential** — an opaque token that identifies the device to the relay for the lifetime of the UUID pool. The device uses this credential to authenticate `GET /sse`, `GET /pending`, and `POST /ack` requests. The credential is distinct from the push token and from UUID values.
...
3. The relay service generates a UUID pool and a **device credential** for this registration. Each UUID is mapped to the push token and wallet base URL (looked up from the app registry via `app_id`).
4. The relay returns the UUID pool and device credential to the device. The relay stores for each UUID:
   ```
   uuid → { app_id, push_token, wallet_base_url, device_credential, status: "unused" }
   ```
...
6. For **each card separately** — in its own session, staggered in time from any other card's registration (§Registration Privacy) — the device registers that card's allocated UUIDs with its wallet service. The registration is a signed envelope proving control of the subcard the UUIDs are being registered for, not a bare list:
   ...
   ```
   This session is conducted over `ObliviousProtocolTransport` (HPKE/OHTTP relay-forwarding per RFC 9180, as implemented by `app_sdk.md §4.7`) — see §Registration Privacy for why this satisfies the anonymizing-transport requirement for this call.
...
3. The wallet service **retains** the message until it receives `DELETE /messages/{uuid}` from the relay (staggered clearance, Process 6). The wallet's copy is the source of truth for undelivered messages.

**Relay → device:**

4. The relay checks whether an SSE connection is open for the device's `device_credential`:
   - **SSE open:** stream `{ "uuid": "<uuid>", "blob": "<base64url>" }` to the device immediately. On acknowledgment (device calls `POST /ack` with the UUID), schedule a staggered delete to the wallet (Process 6) and remove the blob from the message store.
   - **No SSE:** continue to step 5.

5. The relay checks whether a WebSocket session is active for a UUID associated with the same `device_credential`:
...
6. The device acknowledges receipt:
   ...
   Body: { "uuids": ["<uuid>", ...] }
   ```
7. The relay schedules a staggered delete to the wallet service for each acknowledged UUID (Process 6) and removes the blobs from the pending store.
...
- The relay makes one outbound delete call per delivered message, to the `wallet_base_url` stored in the UUID record at registration time.
...
**Transport:** wallet registration sessions (§Process 1, step 6) are conducted over `ObliviousProtocolTransport` (HPKE/OHTTP relay-forwarding per RFC 9180, as implemented by `app_sdk.md §4.7`) by default — this is the expected mechanism, not an opt-in reserved for "users with strong privacy requirements." For `registerCardUuids` specifically, this oblivious-relay transport satisfies the anonymizing-transport requirement on its own; Tor or an equivalent network-level anonymizer is not additionally required for this call (see `oblivious_transport.md` for why HPKE/OHTTP relay-forwarding is sufficient here).

**Transport anonymity and content-level correlation are different protections, and this section's per-card separation requirement exists because of the second one.** The oblivious-relay transport (or Tor, or any other anonymizing transport) hides which network identity sent a registration — it does nothing to stop a wallet service from reading co-ownership directly out of a message's *contents*. A single signed envelope listing multiple `card_hash` values together, even sent through an anonymizing transport, tells the receiving wallet service those cards are held by the same device just as plainly as an unencrypted one would. This is why batching multiple cards' UUID registrations into one session or message is not permitted regardless of transport (see §Process 1 step 6) — anonymizing transport and per-card session separation address different halves of the correlation problem, and dropping either one reopens the co-ownership inference this section exists to prevent.
...
| Push token rotated by platform | Device calls `POST /register` with its existing device credential and the new push token; the relay replenishes in place, updating the `push_token` field on the existing credential's record — the same `device_credential` and already-registered UUIDs remain valid, and no new credential is issued (`relay.md §6.3`, `relay_data_model.md §8.3`) |
