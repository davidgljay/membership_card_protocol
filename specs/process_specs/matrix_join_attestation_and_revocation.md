...
**Open item — wire transport.** This document specifies the attestation's shape and verification, not the exact Matrix API extension point it rides on (a custom `/join` request parameter vs. `m.room.member` event content vs. a pre-join out-of-band call). That binding is a Phase 4 implementation detail, flagged the same way other wire-level open items are flagged elsewhere in this spec set (e.g. `matrix_room.md`'s pointer-resolution-location open item).

---

## 2. Revised Join Sequence (supersedes `matrix_room_membership.md §1`)

Triggered the same way as before — Synapse's `user_may_join_room` callback — but step 2 no longer calls out to `wallet-service`.
...
**Creator auto-join, carried over unchanged.** `matrix_synapse_module.md`'s existing note that `user_may_join_room` isn't invoked for a room creator's own auto-join still applies. A creator's attestation, if a client submits one anyway, can be verified defensively, but it isn't required — the creator is still trusted by virtue of having authenticated to `wallet-service` to create the room in the first place. **The creator's membership must still be entered into the membership registry (§2a)** — since their own `user_may_join_room` callback never fires, whatever code path handles their auto-join has to register the entry directly (from the room-creation call's own known `card_hash`), or their first post would have no registry entry to resolve against.
...
- **Encryption:** the `card_hash` column (and, for simplicity, the row as a whole) is encrypted at rest using a key managed through the **same secrets abstraction already used for every other generated credential in this deployment** (`SECRETS_BACKEND` — webcrypto vs. KMS, per Step 7's pattern for the Synapse signing key and Step 7b's watcher admin token) — not a new, one-off key-management scheme. The module holds the decryption key in memory for as long as it's running, the same way it already holds the watcher's admin-API token and RPC credentials.
- **On startup, reconcile against Synapse's own live room-membership list**, not just load-and-trust the persisted file: for each room-membership Synapse reports, confirm a corresponding registry entry exists. A membership Synapse still lists but the registry has no entry for (the file was lost, corrupted, or predates this feature) is treated exactly like any other registry miss (§4's `"membership_not_registered"` row, deny that specific member's posts until they rejoin) — this is a targeted, per-member gap, not the mass-rejoin-on-every-restart failure mode this design change exists to avoid. A registry entry for a room the member has since left (per Synapse) is pruned — no reason to retain a card-identity binding for a membership that no longer exists, consistent with the protocol's general data-minimization posture.
...
- **Exact wire transport for the join attestation** (§1's open item, restated).
...
| New standing infrastructure | None beyond the module itself | Watcher daemon; persistent RPC subscription; watch-set index; Synapse admin-API access for force-part; **an encrypted, persisted membership registry (`(room_id, matrix_user_id) → card_hash`), surviving restarts** |
