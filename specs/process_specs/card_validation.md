...
12. Verify `app_signature` is valid (using `app_card_pubkey` already in hand from step 7b). The runtime verifier does **not** re-walk the `app_card` certification chain to the governance root — that walk was performed by the **press at registration time** before `RegisterSubCard` was submitted (see `subcards.md §Step 5` and `registry_contract.md §4.3`). The `sub_card_doc_cid` on the on-chain `SubCardEntry` is evidence that the press accepted and registered this document; runtime verifiers rely on the press's app-chain validation. The `app_card_pubkey` field in the `SubCardDocument` is available for auditors and the press; it is not used to re-walk the app-certification chain during runtime verification.
13. If any link in Stage 2 cannot be confirmed, record `scope_clean: false`.

### Stage 3: Chain Walk (Historical Validity)

14. Read `ancestry_pubkeys` from the now-decrypted master card (obtained in Stage 2 step 8). This array is set at master card issuance, covered by all three of the master card's signatures, and ordered from immediate parent up toward root. The walk from the master card to the trusted root proceeds entirely via the master card's own `ancestry_pubkeys` — the sub-card's `holder_primary_card_pubkey` field (§16) served to unlock the master card at the sub-card boundary; subsequent ancestor hops use those cards' own `ancestry_pubkeys` fields. Note: the `app_card_pubkey` field in `SubCardDocument` is NOT used in the Stage 3 walk — the app-certification chain walk is press-side only (see Stage 2 step 12).
...
| `app_card_pubkey` binding check fails (`keccak256(app_card_pubkey)` ≠ `app_card` address) | Hard rejection — record `scope_clean: false` and abort Stage 2. The hint is forged or incorrect. (Note: the runtime verifier checks `app_card_pubkey` only to verify `app_signature` at step 12 — it does not decrypt the app card or re-walk the app-certification chain.) |
