...
`relay-next/` is the serverless rebuild of the [notification relay](../relay/README.md):
...
**Authoritative source: [`relay-next/PROVISIONING.md`](./PROVISIONING.md).**
...
[`relay/README.md`](../relay/README.md)'s "App registry config" section
for the JSON schema itself, which `relay-next` reuses). What changed is
...
[`relay-next/wrangler.toml`](./wrangler.toml) is the production Worker
...
A second config, [`relay-next/wrangler.do-test.toml`](./wrangler.do-test.toml),
...
[`relay-next/spike-do-ws/wrangler.toml`](./spike-do-ws/wrangler.toml), is
...
Full detail in [`relay-next/spike-do-ws/README.md`](./spike-do-ws/README.md);
summarized here:
...
- **No real `wrangler deploy` to a live Cloudflare account has been
  ...
  `relay-next` Worker itself has not yet gone through a first real
  deploy, so there may be more to find.
- **Real Cloudflare Durable Object hibernation-eviction timing is
  unmeasured.** `relay-next/spike-do-ws/test-hibernation.mjs` exists and
